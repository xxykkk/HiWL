...
class RandomJpegSS(JpegBasic):
	"""
	JPEG-SS with random compression quality parameter
	"""
	def __init__(self, min_quality=50, max_quality=100, subsample=2, padding = False):
		super(RandomJpegSS, self).__init__()
		self.min_quality = min_quality
		self.max_quality = max_quality
		# subsample
		self.subsample = subsample
...
class RandomJpegMask(JpegBasic):
	"""
	JPEGMask with random compression quality parameter
	"""
	def __init__(self, min_quality=50, max_quality=100, subsample=2, padding = False):
		super(RandomJpegMask, self).__init__()
		self.min_quality = min_quality
		self.max_quality = max_quality
		self.padding = padding
...
class MyJpegTest(JpegBasic):
	...
	def std_quantization(self, image_yuv_dct, scale_factor, round_func=torch.round):
		...
		temp_int = torch.zeros_like(image_yuv_dct)
		temp_int[:, :1, :, :] = (image_yuv_dct[:, :1, :, :].detach() / luminance_quant_tbl).round()
		temp_int[:, 1:, :, :] = (image_yuv_dct[:, 1:, :, :].detach() / chrominance_quant_tbl).round()

		dynamic_luminance_quant_tbl   = image_yuv_dct[:, :1, :, :].detach()/temp_int[:, :1, :, :]
		dynamic_chrominance_quant_tbl = image_yuv_dct[:, 1:, :, :].detach()/temp_int[:, 1:, :, :]
		
		q_image_yuv_dct = image_yuv_dct.clone()
		q_image_yuv_dct[:, :1, :, :] = image_yuv_dct[:, :1, :, :] / dynamic_luminance_quant_tbl
		q_image_yuv_dct[:, 1:, :, :] = image_yuv_dct[:, 1:, :, :] / dynamic_chrominance_quant_tbl

		q_image_yuv_dct_round=q_image_yuv_dct
		return q_image_yuv_dct_round