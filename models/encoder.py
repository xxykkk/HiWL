# *************************************************************************
# Code Modified from https://github.com/bytedance/DWSF/blob/main/networks/models/Encoder.py
# *************************************************************************
...
class Encoder_v2(nn.Module):
    ...
    """
    def __init__(self, H=128, W=128, message_length=64, encoder_channels=256, encoder_blocks=4, in_channel=3, out_channel=3):
        super().__init__()
        self.H = H
        self.W = W
        self.conv_channels = encoder_channels
        self.num_blocks = encoder_blocks
        self.message_size = int(encoder_channels ** 0.5) # 8
        ...
        self.after_concat_layer5 = nn.Sequential(
            SENet(self.conv_channels + encoder_channels, self.conv_channels + encoder_channels, blocks=1),
            SENet_decoder(self.conv_channels + encoder_channels, self.conv_channels,blocks=2,drop_rate2=1),
            SENet(self.conv_channels, self.conv_channels, blocks=encoder_blocks),
            ExpandNet(self.conv_channels, self.conv_channels, blocks=1)
        ...
        self.after_concat_layer6 = nn.Sequential(
    ...
    def forward(self, image, message):
        ...
        concat4 = torch.cat([message_pre, x], dim=1)
        x = self.after_concat_layer4(concat4)

        concat5 = torch.cat([message_pre, x], dim=1)
        x = self.after_concat_layer5(concat5)

        concat6 = torch.cat([image, x], dim=1)
        x = self.after_concat_layer6(concat6)


        concat_out = torch.cat([image, x], dim=1)
        x = self.final_layer(concat_out)
